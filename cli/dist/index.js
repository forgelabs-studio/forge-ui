...
// Re-generate with: npx @forgelabs-studio/ui update button
...
`));
  try {
    ...
    if (!config) {
      console.log(
        pc5.red("  No forge.config.json found. Run npx @forgelabs-studio/ui init first.\n")
      );
      process.exit(1);
    }
    ...
    if (!savedProps) {
      ...
      console.log(
        pc5.dim("  Run npx @forgelabs-studio/ui add " + componentId + " first.\n")
      );
      process.exit(1);
    }
    const meta = REGISTRY_BY_ID[componentId];
...
program.name("forge-ui").description("FORGE.ui \u2014 spectrum-aware React component library").version("0.5.0");
