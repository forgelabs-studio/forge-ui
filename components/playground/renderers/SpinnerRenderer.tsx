...
export default function SpinnerRenderer({ props: p }: { props: SpinnerProps }) {
  ...
  const sz =
    ({ sm: 24, md: 36, lg: 48, xl: 64 } as Record<string, number>)[
      p.size ?? "md"
    ] ?? 36;
  const dur =
  ...
  const ring = (
    <div
      ...
      style={{
        ...
        height: sz,
        borderRadius: "50%",
        border: `${Math.max(1, Math.round(sz * 0.1))}px solid rgba(${rgb},.15)`,
        borderTopColor: col,
  ...
  return (
    ...
    >
      ...
      {/* Visible label is optional; keep it accessible as well */}
      {p.showLabel && (
        <span style={{ fontSize: 12, color: "rgba(240,237,232,.4)" }}>
          {label}
        </span>
      )}
    </div>
