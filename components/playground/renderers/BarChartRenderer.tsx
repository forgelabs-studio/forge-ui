...
export default function BarChartRenderer({ props: p }: { props: any }) {
  ...
  const labels = hasData ? rawData.map((s:string)=>s.split(',')[0]||'') : ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug']
  const vals1  = hasData ? rawData.map((s:string)=>Number(s.split(',')[1])||0) : [8,12,9,15,11,18,14,20]
  const vals2  = hasData ? rawData.map((s:string)=>Number(s.split(',')[2])||0) : [12,18,14,22,17,26,21,30]
  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      data: {
        labels,
        datasets: [
          {label:'2024',data:vals1,backgroundColor:`rgba(${rgb},.35)`,borderColor:`rgba(${rgb},.6)`,borderWidth:1,borderRadius:p.radius??4},
